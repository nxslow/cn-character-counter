...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(disposable, textChangeListener, saveListener);
}
...
function countCharactersInActiveDocument() {
    ...
    vscode.window.showInformationMessage('Character count completed!');
}
    ...
    try {
        ...
        if (edits.length > 0) {
            ...
            if (editor) {
                ...
                vscode.window.showInformationMessage(`Added ${edits.length} separator line(s)`);
            }
        }
    } catch (error) {
        console.error('Error in character counter:', error);
        vscode.window.showErrorMessage('Character counter encountered an error');
    }
}
